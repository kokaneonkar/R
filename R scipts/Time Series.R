...
#day wise Vaccination rate

vday<-newdta[,c(1,3)]
View(vday)
vday$Updated.On<-as.Date(vday$Updated.On)
vday$Updated.On<-format(vday$Updated.On,"%A")
vday1<-aggregate(vday$Total.Individuals.Vaccinated~vday$Updated.On,FUN=sum)
View(vday1)
summary(vday1)
vdday<-barplot(vday1$`vday$Total.Individuals.Vaccinated`, las=2, names.arg = vday1$`vday$Updated.On`, 
               col=c(rgb(0.3,0.1,0.4,0.6) , rgb(0.3,0.5,0.4,0.6) , 
                     rgb(0.3,0.9,0.4,0.6) , rgb(0.3,0.9,0.4,0.6),
                     rgb(0.3,0.9,0.4,0.6), rgb(0.3,0.1,0.4,0.6), 
                     rgb(0.3,0.9,0.4,0.6)),
               xlab = "Day", ylab="no. of vaccinations", main = "Day wise Vaccination rate")
text(vdday,vday$Total.Individuals.Vaccinated+0.4,paste(vday1$`vday$Total.Individuals.Vaccinated`,sep = ""),cex = 0.9)
